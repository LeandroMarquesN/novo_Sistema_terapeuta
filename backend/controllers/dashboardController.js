...
// =============================================================================
// 1. ESTATÍSTICAS DOS CARDS DO DASHBOARD (API — retorna JSON)
// =============================================================================
async function getEstatisticasHoje(req, res) {
  ...
  try {
    // ── 1. Atendimentos hoje (excluindo cancelados) ──
    const [[{ total: atendimentosHoje }]] = await db.query(
      `SELECT COUNT(*) as total FROM agendamentos
       WHERE clinica_id = ? AND DATE(data_agendamento) = CURDATE()
       AND status_agendamento != 'cancelado'`,
      [clinicaId]
    );

    const [[{ total: atendimentosOntem }]] = await db.query(
      `SELECT COUNT(*) as total FROM agendamentos
       WHERE clinica_id = ? AND DATE(data_agendamento) = DATE_SUB(CURDATE(), INTERVAL 1 DAY)
       AND status_agendamento != 'cancelado'`,
      [clinicaId]
    );

    let variacaoAtendimentos = 0;
    if (atendimentosOntem > 0) {
      variacaoAtendimentos = Math.round(((atendimentosHoje - atendimentosOntem) / atendimentosOntem) * 100);
    } else if (atendimentosHoje > 0) {
      variacaoAtendimentos = 100;
    }

    // ── 2. Cancelamentos hoje + taxa ──
    const [[{ total: cancelamentosHoje }]] = await db.query(
      `SELECT COUNT(*) as total FROM agendamentos
       WHERE clinica_id = ? AND DATE(data_agendamento) = CURDATE()
       AND status_agendamento = 'cancelado'`,
      [clinicaId]
    ...
    const totalAgendamentosHoje = atendimentosHoje + cancelamentosHoje;
    const taxaCancelamento = totalAgendamentosHoje > 0
      ? ((cancelamentosHoje / totalAgendamentosHoje) * 100).toFixed(1)
      : '0.0';

    // ── 3. Faturamento hoje (pagamentos liquidados) ──
    const [[{ total_faturado: faturamentoHoje, qtd_sessoes: sessoesLiquidadas }]] = await db.query(
      `SELECT
    ...
    // ── 4. Sessões agendadas nos próximos 7 dias (confirmado + aguardando_sinal) ──
    // Conta a partir de amanhã até 7 dias à frente — hoje já está representado no card 1
    const [[{ total: proximaSemana }]] = await db.query(
      `SELECT COUNT(*) as total FROM agendamentos
       WHERE clinica_id = ?
       AND DATE(data_agendamento) BETWEEN DATE_ADD(CURDATE(), INTERVAL 1 DAY) AND DATE_ADD(CURDATE(), INTERVAL 7 DAY)
       AND status_agendamento IN ('confirmado', 'aguardando_sinal')`,
      [clinicaId]
    );
    ...
    res.json({
      success: true,
      atendimentos: {
        total: atendimentosHoje,
        variacao_percentual: variacaoAtendimentos
      },
      cancelamentos: {
        total: cancelamentosHoje,
        taxa_percentual: parseFloat(taxaCancelamento)
      },
      faturamento: {
        total: parseFloat(faturamentoHoje),
        sessoes_liquidadas: sessoesLiquidadas
      },
      proxima_semana: {
        total: proximaSemana
      }
    ...
    res.status(500).json({ success: false, message: "Erro ao carregar estatísticas.", details: error.message });
  }
}

// =============================================================================
// 2. RENDERIZAÇÃO DA PÁGINA DO DASHBOARD (tabela, gráficos, calendário)
// =============================================================================
...
// =============================================================================
// EXPORT — um único objeto com os dois métodos, sem o bug do operador vírgula
// =============================================================================
module.exports = {
  index,
  getEstatisticasHoje
