  ...
  try {
    ...
    if (!paciente_id || !modelo_id || !respostas) {
      return res.status(400).json({
        erro: 'paciente_id, modelo_id e respostas são obrigatórios.'
      });
    }
    ...
    // Auto-vínculo pelo mesmo agendamento
    ...
    const respostasJson =
      typeof respostas === 'string' ? respostas : JSON.stringify(respostas);
    ...
    if (id) {
      ...
      if (result.affectedRows === 0) {
        return res.status(403).json({
          erro: 'Anamnese não encontrada ou já finalizada.'
        });
      }
      return res.json({
        ok: true,
...
/**
 * Vincular anamnese a um prontuário
...
exports.vincularProntuario = async (req, res) => {
  try {
    ...
    const [aRows] = await db.execute(
      `SELECT id, paciente_id FROM anamneses_preenchidas
    ...
    const [pRows] = await db.execute(
      `SELECT id FROM prontuarios
       WHERE id = ? AND clinica_id = ? AND paciente_id = ? LIMIT 1`,
      [prontuario_id, clinicaId, aRows[0].paciente_id]
    );
    if (!pRows.length) {
      return res.status(400).json({
        erro: 'Prontuário não pertence a este paciente.'
      });
    }
